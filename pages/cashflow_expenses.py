...
# Receipt and payment ShortName codes from MsTransType
RECEIPT_IN = "'BR','CR'"
PAYMENT_IN = "'BP','CE'"

# DrCrIndicator='D' on a receipt voucher = bank/cash account debited = money received.
# DrCrIndicator='D' on a payment voucher = expense/supplier account debited = money paid out.
COLL_INDICATOR = "D"
PAY_INDICATOR  = "D"
...
def render():
    ...
    kpi = query(f"""
        SELECT
            SUM(CASE WHEN t.ShortName IN ({RECEIPT_IN}) AND d.DrCrIndicator='{COLL_INDICATOR}'
                THEN d.Amount ELSE 0 END)  AS total_collections,
            SUM(CASE WHEN t.ShortName IN ({PAYMENT_IN}) AND d.DrCrIndicator='{PAY_INDICATOR}'
    ...
    with col_l:
        ...
        # SalesManID is on TrVocHead — no need for party from TrVocDetail
        df_sm_coll = query(f"""
            ...
            WHERE t.ShortName IN ({RECEIPT_IN})
              AND d.DrCrIndicator='{COLL_INDICATOR}'
    ...
    with col_r:
        ...
        df_pay_type = query(f"""
            ...
            WHERE t.ShortName IN ({PAYMENT_IN})
              AND d.DrCrIndicator='{PAY_INDICATOR}'
              AND ISNULL(h.Cancelled,'N') <> 'Y'
